...
public class LadyFortune_PayOut_BetType_3_And_Denomination_All {
	AppiumDriver<MobileElement> driver;

	public LadyFortune_PayOut_BetType_3_And_Denomination_All() throws InterruptedException {
		this.driver = LadyFortune_URL_Login.getDriver();
		}
	
	@Given("^Chrome browser, valid URL, valid login details, Lady Fortune slot game, icon to open payout table, all symbols, max amount on each symbol like Q, K, (\\d+) n etc, back button, bet type (\\d+)\\.(\\d+) and all denomination values  on mobile$")
	public void chrome_browser_valid_URL_valid_login_details_Lady_Fortune_slot_game_icon_to_open_payout_table_all_symbols_max_amount_on_each_symbol_like_Q_K_n_etc_back_button_bet_type_and_all_denomination_values_on_mobile(int arg1, int arg2, int arg3) throws Throwable {
	...
	@When("^Open the Lady Fortune slot game by entering the valid URL in browser, enter the valid login details, transfer the balance, click on icon to open payout table, check payout amount on each symbol like Q, K, (\\d+) n etc, click on back button, select bet type as (\\d+)\\.(\\d+) & all denomination  values and check payout amount on mobile$")
	public void open_the_Lady_Fortune_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_transfer_the_balance_click_on_icon_to_open_payout_table_check_payout_amount_on_each_symbol_like_Q_K_n_etc_click_on_back_button_select_bet_type_as_all_denomination_values_and_check_payout_amount_on_mobile(int arg1, int arg2, int arg3) throws Throwable {
	...
	@Then("^System should display the Payout max amount on each symbol like Q, K, (\\d+) n etc and payout max amount should get changed whenever player changes the bet type to (\\d+)\\.(\\d+) and all denomination values in Lady Fortune slot game on mobile$")
	public void system_should_display_the_Payout_max_amount_on_each_symbol_like_Q_K_n_etc_and_payout_max_amount_should_get_changed_whenever_player_changes_the_bet_type_to_and_all_denomination_values_in_Lady_Fortune_slot_game_on_mobile(int arg1, int arg2, int arg3) throws Throwable {
	    Thread.sleep(2000);
	    driver.quit();
	}
}
