...
public class LadyFortune_PayOut_BetType_1_And_Denomination_All {
	AppiumDriver<MobileElement> driver;

	public LadyFortune_PayOut_BetType_1_And_Denomination_All() throws InterruptedException {
	...
	@Then("^System should display the Payout max amount on each symbol and payout max amount should get changed whenever player changes the bet type to (\\d+)\\.(\\d+) and denomination values in Lady Fortune slot game on mobile$")
	public void system_should_display_the_Payout_max_amount_on_each_symbol_and_payout_max_amount_should_get_changed_whenever_player_changes_the_bet_type_to_and_denomination_values_in_Lady_Fortune_slot_game_on_mobile(int arg1, int arg2) throws Throwable {
	    Thread.sleep(2000);
	    driver.quit();
	}
}
