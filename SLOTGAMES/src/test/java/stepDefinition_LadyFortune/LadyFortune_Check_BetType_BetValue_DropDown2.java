...
public class LadyFortune_Check_BetType_BetValue_DropDown2 {
	AppiumDriver<MobileElement> driver;

	public LadyFortune_Check_BetType_BetValue_DropDown2() throws InterruptedException {
	...
	@Then("^Bet value should get changed to (\\d+)\\.(\\d+),(\\d+),(\\d+)\\.(\\d+),(\\d+),(\\d+) if the bet type selected as (\\d+)\\.(\\d+) from the drop down (\\d+) in Lady Fortune slot game$")
	public void bet_value_should_get_changed_to_if_the_bet_type_selected_as_from_the_drop_down_in_Lady_Fortune_slot_game(int arg1, int arg2, int arg3, int arg4, int arg5, int arg6, int arg7, int arg8, int arg9, int arg10) throws Throwable {
	    Thread.sleep(2000);
	    driver.quit();
	}
}
