...
public class PerfectGems_Gamble_GambleCount_For_BetType_1_And_Denomination_1 {
	AppiumDriver<MobileElement> driver;

	public PerfectGems_Gamble_GambleCount_For_BetType_1_And_Denomination_1() throws InterruptedException {
	...
	@When("^Open the Perfect Gems slot game by entering the valid URL in browser, enter the valid login details, transfer the balance, select bet type as (\\d+)\\.(\\d+) & denomination as (\\d+)\\.(\\d+), click on spin button till player wins, click on gamble button and check the gamble count$")
	public void open_the_Perfect_Gems_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_transfer_the_balance_select_bet_type_as_denomination_as_click_on_spin_button_till_player_wins_click_on_gamble_button_and_check_the_gamble_count(int arg1, int arg2, int arg3, int arg4) throws Throwable {
		...
		Thread.sleep(1000);
		MobileElement bet1_5= driver.findElement(By.id("hud_BetPopup10.25"));
		String Betval1_5 =bet1_5.getText();
		System.out.println(Betval1_5);
		String expectedB = "0.25";
		...
		System.out.println("Maximum gamble win amount for bet amount 0.25 and credit value 0.01 is : 8.75 SRD");	                                                                                                                             
		Double maxV = Double.parseDouble(prewin);
		if(maxV < 8.75)
		   {
			   System.out.println("Win amount less than Gamble max value 8.75 i.e : "+" " +maxV +". Test Case Passed");
		   }
		else
		{
			System.out.println("Win amount greater than Gamble max value 8.75 : i.e "+" " +maxV +". Test Case Failed");
	...
	@Then("^Gamble count should be displayed based on win amount and gamble max amount configured on the game info page for bet type (\\d+)\\.(\\d+) & denomination (\\d+)\\.(\\d+) in Perfect Gems game$")
	public void gamble_count_should_be_displayed_based_on_win_amount_and_gamble_max_amount_configured_on_the_game_info_page_for_bet_type_denomination_in_Perfect_Gems_game(int arg1, int arg2, int arg3, int arg4) throws Throwable {
		Thread.sleep(1000);
	    driver.quit();
	}
}
