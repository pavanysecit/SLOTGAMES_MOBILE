...
public class FourtySplendidHot_Balance_Deduction_AccordingToBetType2_4 {
	AppiumDriver<MobileElement> driver;

	public FourtySplendidHot_Balance_Deduction_AccordingToBetType2_4() throws InterruptedException {
	...
	@When("^Open the (\\d+) Splendid Hot slot game by entering the valid URL in browser, enter the valid login details, select the bet type as (\\d+)\\.(\\d+), select the denomination as EIGHT, click on spin button and check the balance$")
	public void open_the_Splendid_Hot_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_select_the_bet_type_as_select_the_denomination_as_EIGHT_click_on_spin_button_and_check_the_balance(int arg1, int arg2, int arg3) throws Throwable {
	...
	@Then("^Current Balance should get deducted by (\\d+)\\.(\\d+) as bet type is selected as (\\d+)\\.(\\d+) & bet value as EIGHT in (\\d+) Splendid Hot game$")
	public void current_Balance_should_get_deducted_by_as_bet_type_is_selected_as_bet_value_as_EIGHT_in_Splendid_Hot_game(int arg1, int arg2, int arg3, int arg4, int arg5) throws Throwable {
		Thread.sleep(2000);
	    driver.quit();
	}
}
