...
public class FourtySplendidHot_PayOut_BetType_1_And_Denomination_2 {
	AppiumDriver<MobileElement> driver;

	public FourtySplendidHot_PayOut_BetType_1_And_Denomination_2() throws InterruptedException {
	...
	@When("^Open the (\\d+) Splendid Hot slot game by entering the valid URL in browser, enter the valid login details, transfer the balance, click on icon to open payout table, check payout amount on each symbol like lemon & etc, click on back button, select bet type as (\\d+)\\.(\\d+) & deominations as (\\d+)\\.(\\d+) and check payout amount$")
	public void open_the_Splendid_Hot_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_transfer_the_balance_click_on_icon_to_open_payout_table_check_payout_amount_on_each_symbol_like_lemon_etc_click_on_back_button_select_bet_type_as_deominations_as_and_check_payout_amount(int arg1, int arg2, int arg3, int arg4, int arg5) throws Throwable {
		...
		if(me.getId().equals("32")){
			...
			System.out.println("Payout Value of 5 Grapes symbol for denomination 0.8 is: " +Grapes);
		...
		if(me.getId().equals("33")){
			...
			System.out.println("Payout Value of 4 Grapes symbol for denomination 0.8 is: " +Grapes1);
		...
		if(me.getId().equals("34")){
			...
			System.out.println("Payout Value of 3 Grapes symbol for denomination 0.8 is: " +Grapes2);
		...
		if(me.getId().equals("35")){
			...
			System.out.println("Payout Value of 5 Plum or Watermelon symbol for denomination 0.8 is: " +Plum);
		...
		if(me.getId().equals("36")){
			...
			System.out.println("Payout Value of 4 Plum or Watermelon symbol for denomination 0.8 is: " +Plum1);
		...
		if(me.getId().equals("37")){
			...
			System.out.println("Payout Value of 3 Plum or Watermelon symbol for denomination 0.8 is: " +Plum2);
		...
		if(me.getId().equals("38")){
			...
			System.out.println("Payout Value of 5 Orange or Lemon or Cherry symbol for denomination 0.8 is: " +Orange);
		...
		if(me.getId().equals("39")){
			...
			System.out.println("Payout Value of 4 Orange or Lemon or Cherry symbol for denomination 0.8 is: " +Orange1);
		...
		if(me.getId().equals("40")){
			...
			System.out.println("Payout Value of 3 Orange or Lemon or Cherry symbol for denomination 0.8 is: " +Orange2);
	...
	@Then("^System should display the Payout max amount on each symbol like lemon & etc and payout max amount should get changed whenever player changes the bet type to (\\d+)\\.(\\d+) and deomination to (\\d+)\\.(\\d+) in (\\d+) Splendid Hot slot game on mobile$")
	public void system_should_display_the_Payout_max_amount_on_each_symbol_like_lemon_etc_and_payout_max_amount_should_get_changed_whenever_player_changes_the_bet_type_to_and_deomination_to_in_Splendid_Hot_slot_game_on_mobile(int arg1, int arg2, int arg3, int arg4, int arg5) throws Throwable {
		Thread.sleep(2000);
	    driver.quit();
	}
}
