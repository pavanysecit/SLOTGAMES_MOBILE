...
public class FourtySplendidHot_Balance_Check_MaxBet_WinAmount_AddedTo_Balance {
	AppiumDriver<MobileElement> driver;

	public FourtySplendidHot_Balance_Check_MaxBet_WinAmount_AddedTo_Balance() throws InterruptedException {
		this.driver = FourtySplendidHot_URL_Login.getDriver();
		//this.driver = FourtySplendidHot_URL_TryNow.getDriver();
		}
	
	@Given("^Chrome browser, valid URL, valid login details, (\\d+) Splendid Hot slot game, balance, spin button, gamble collect, max credit and bet value, win amount added to balance$")
	public void chrome_browser_valid_URL_valid_login_details_Splendid_Hot_slot_game_balance_spin_button_gamble_collect_max_credit_and_bet_value_win_amount_added_to_balance(int arg1) throws Throwable {
	    
	}

	@When("^Open the (\\d+) Splendid Hot slot game by entering the valid URL in browser, enter the valid login details, spin till player wins, gamble screen, gamble collect, win amount added to main balance$")
	public void open_the_Splendid_Hot_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_spin_till_player_wins_gamble_screen_gamble_collect_win_amount_added_to_main_balance(int arg1) throws Throwable {
	...
	@Then("^MaxBet Win amount should get added to the main balance after win and balance should get increased with win amount in (\\d+) Splendid Hot slot game$")
	public void maxbet_Win_amount_should_get_added_to_the_main_balance_after_win_and_balance_should_get_increased_with_win_amount_in_Splendid_Hot_slot_game(int arg1) throws Throwable {
		Thread.sleep(2000);
	    driver.quit();
	}
}
