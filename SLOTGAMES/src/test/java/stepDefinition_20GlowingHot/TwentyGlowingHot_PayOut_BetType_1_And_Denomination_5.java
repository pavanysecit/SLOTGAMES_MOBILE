...
public class TwentyGlowingHot_PayOut_BetType_1_And_Denomination_5 {
	AppiumDriver<MobileElement> driver;

	public TwentyGlowingHot_PayOut_BetType_1_And_Denomination_5() throws InterruptedException {
		this.driver = TwentyGlowingHot_URL_Login.getDriver();
		//this.driver = TwentyGlowingHot_URL_TryNow.getDriver();
		}
	
	@Given("^Chrome browser, valid URL, valid login details, (\\d+) Glowing Hot slot game, icon to open payout table, all symbols, max amount on each symbol like grapes, plum & etc, back button, bet type (\\d+)\\.(\\d+) and denomination as FOUR on mobile$")
	public void chrome_browser_valid_URL_valid_login_details_Glowing_Hot_slot_game_icon_to_open_payout_table_all_symbols_max_amount_on_each_symbol_like_grapes_plum_etc_back_button_bet_type_and_denomination_as_FOUR_on_mobile(int arg1, int arg2, int arg3) throws Throwable {
		WebDriverWait wait = new WebDriverWait(driver, 90);
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("hud_Hud_txtBalance1")));
	}

	@When("^Open the (\\d+) Glowing Hot slot game by entering the valid URL in browser, enter the valid login details, transfer the balance, click on icon to open payout table, check payout amount on each symbol like grapes, plum & etc, click on back button, select bet type as (\\d+)\\.(\\d+) & denomination as FOUR and check payout amount on mobile$")
	public void open_the_Glowing_Hot_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_transfer_the_balance_click_on_icon_to_open_payout_table_check_payout_amount_on_each_symbol_like_grapes_plum_etc_click_on_back_button_select_bet_type_as_denomination_as_FOUR_and_check_payout_amount_on_mobile(int arg1, int arg2, int arg3) throws Throwable {
	...
	@Then("^System should display the Payout max amount on each symbol like grapes, plum & etc and payout max amount should get changed whenever player changes the bet type to (\\d+)\\.(\\d+) and denomination to FOUR in (\\d+) Glowing Hot slot game on mobile$")
	public void system_should_display_the_Payout_max_amount_on_each_symbol_like_grapes_plum_etc_and_payout_max_amount_should_get_changed_whenever_player_changes_the_bet_type_to_and_denomination_to_FOUR_in_Glowing_Hot_slot_game_on_mobile(int arg1, int arg2, int arg3) throws Throwable {
		Thread.sleep(3000);
		driver.quit();
	}
}
