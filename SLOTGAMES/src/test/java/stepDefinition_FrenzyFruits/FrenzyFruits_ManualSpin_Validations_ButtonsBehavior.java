...
public class FrenzyFruits_ManualSpin_Validations_ButtonsBehavior {
	AppiumDriver<MobileElement> driver;

	public FrenzyFruits_ManualSpin_Validations_ButtonsBehavior() throws InterruptedException {
	...
	public void chrome_browser_valid_URL_valid_login_details_Frenzy_Fruits_slot_game_balance_spin_button_spin_button_and_check_for_buttons_behavior_functionality() throws Throwable {
	...
	public void open_the_Frenzy_Fruits_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_transfer_the_balance_click_on_spin_option_and_check_behavior_of_different_buttons_in_different_scenarios() throws Throwable {
		...
		Thread.sleep(2000);
		if(driver.findElements(By.id("hud_CreditPopup10.02")).size() != 0){
		...
		if(driver.findElements(By.id("hud_BetPopup20.4")).size() != 0){
			Assert.assertTrue(driver.findElements(By.id("hud_BetPopup20.8")).size() != 0);
	...
	public void bet_value_Credit_value_buttons_should_not_be_clickable_under_spin_condition_in_Frenzy_Fruits_slot_game() throws Throwable {
		Thread.sleep(1000);
	}

	@Then("^Home, Info and Turbo buttons should not be clickable on screen under spin condition in Frenzy Fruits slot game$")
	public void home_Info_and_Turbo_buttons_should_not_be_clickable_on_screen_under_spin_condition_in_Frenzy_Fruits_slot_game() throws Throwable {
		Thread.sleep(1000);
	}

	@Then("^Spin button should be disabled during reel spin in Frenzy Fruits slot game$")
	public void spin_button_should_be_disabled_during_reel_spin_in_Frenzy_Fruits_slot_game() throws Throwable {
		Thread.sleep(1000);
	}

	@Then("^Spin button should be visible after reel spin stops and if  win is triggered clicking on collect button in Frenzy Fruits slot game$")
	public void spin_button_should_be_visible_after_reel_spin_stops_and_if_win_is_triggered_clicking_on_collect_button_in_Frenzy_Fruits_slot_game() throws Throwable {
		Thread.sleep(1000);
	}

	@Then("^Gamble button behaviour under win triggered and disabled when next spin starts in Frenzy Fruits slot game$")
	public void gamble_button_behaviour_under_win_triggered_and_disabled_when_next_spin_starts_in_Frenzy_Fruits_slot_game() throws Throwable {
		driver.quit();
	}
}