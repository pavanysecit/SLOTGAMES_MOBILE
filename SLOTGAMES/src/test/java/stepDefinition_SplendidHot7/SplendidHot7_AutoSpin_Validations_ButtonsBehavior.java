...
public class SplendidHot7_AutoSpin_Validations_ButtonsBehavior {
	AppiumDriver<MobileElement> driver;

	public SplendidHot7_AutoSpin_Validations_ButtonsBehavior() throws InterruptedException {
		this.driver = SplendidHot7_URL_Login.getDriver();
		//this.driver = SplendidHot7_URL_TryNow.getDriver();
	}
	
	@Given("^Chrome browser, valid URL, valid login details, Splendid Hot Seven game, balance, spin button, auto spin button, auto spins option and check for buttons behavior & functionality$")
	public void chrome_browser_valid_URL_valid_login_details_Splendid_Hot_Seven_game_balance_spin_button_auto_spin_button_auto_spins_option_and_check_for_buttons_behavior_functionality() throws Throwable {
	   
	}

	@When("^Open the Splendid Hot Seven game by entering the valid URL in browser, enter the valid login details, transfer the balance, click on auto spin option under auto spin drop down and check behavior of different buttons in different scenarios$")
	public void open_the_Splendid_Hot_Seven_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_transfer_the_balance_click_on_auto_spin_option_under_auto_spin_drop_down_and_check_behavior_of_different_buttons_in_different_scenarios() throws Throwable {
	...
	@Then("^Bet value & Credit value buttons should not be clickable in (\\d+) Splendid hot slot game$")
	public void bet_value_Credit_value_buttons_should_not_be_clickable_in_Splendid_hot_slot_game(int arg1) throws Throwable {
		Thread.sleep(1000);
	}

	@Then("^Home and Info buttons should not be clickable in (\\d+) Splendid hot slot game$")
	public void home_and_Info_buttons_should_not_be_clickable_in_Splendid_hot_slot_game(int arg1) throws Throwable {
		Thread.sleep(1000);
	}

	@Then("^Spin button should not visible & Stop button should be visible and clickable in (\\d+) Splendid hot slot game$")
	public void spin_button_should_not_visible_Stop_button_should_be_visible_and_clickable_in_Splendid_hot_slot_game(int arg1) throws Throwable {
		Thread.sleep(1000);
	}

	@Then("^Spin button should be visble after clicking on stop button in (\\d+) Splendid hot slot game$")
	public void spin_button_should_be_visble_after_clicking_on_stop_button_in_Splendid_hot_slot_game(int arg1) throws Throwable {
		driver.quit();
	}
}
