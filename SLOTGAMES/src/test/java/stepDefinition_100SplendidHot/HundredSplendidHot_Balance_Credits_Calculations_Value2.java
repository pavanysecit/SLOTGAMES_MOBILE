...
public class HundredSplendidHot_Balance_Credits_Calculations_Value2 {
	AppiumDriver<MobileElement> driver;

	public HundredSplendidHot_Balance_Credits_Calculations_Value2() throws InterruptedException {
	...
	@When("^Open the (\\d+) Splendid Hot slot game by entering the valid URL in browser, enter the valid login details, click on balance, select the denomination from drop down, multiply credit by (\\d+)\\.(\\d+) and check the balance$")
	public void open_the_Splendid_Hot_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_click_on_balance_select_the_denomination_from_drop_down_multiply_credit_by_and_check_the_balance(int arg1, int arg2, int arg3) throws Throwable {
	...
	@Then("^Balance amound should be same as denomination (\\d+)\\.(\\d+) multiplies by credit in (\\d+) Splendid Hot game$")
	public void balance_amound_should_be_same_as_denomination_multiplies_by_credit_in_Splendid_Hot_game(int arg1, int arg2, int arg3) throws Throwable {
		Thread.sleep(2000);
	    driver.quit();
	}
