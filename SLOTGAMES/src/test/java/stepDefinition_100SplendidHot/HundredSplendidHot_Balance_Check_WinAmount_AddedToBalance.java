...
public class HundredSplendidHot_Balance_Check_WinAmount_AddedToBalance {
	AppiumDriver<MobileElement> driver;

	public HundredSplendidHot_Balance_Check_WinAmount_AddedToBalance() throws InterruptedException {
	...
	@When("^Open the (\\d+) Splendid Hot slot game by entering the valid URL in browser, enter the valid login details, transfer the amount, click on spin button till user win and check the balance after win in (\\d+) Splendid Hot slot game$")
	public void open_the_Splendid_Hot_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_transfer_the_amount_click_on_spin_button_till_user_win_and_check_the_balance_after_win_in_Splendid_Hot_slot_game(int arg1, int arg2) throws Throwable {
	...
	@Then("^Win amount should get added to the balance after win and balance should get increased with win amount in (\\d+) Splendid Hot slot game$")
	public void win_amount_should_get_added_to_the_balance_after_win_and_balance_should_get_increased_with_win_amount_in_Splendid_Hot_slot_game(int arg1) throws Throwable {
		Thread.sleep(2000);
	    driver.quit();
	}
}
