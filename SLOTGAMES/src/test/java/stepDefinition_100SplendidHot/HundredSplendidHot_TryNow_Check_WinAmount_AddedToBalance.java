...
public class HundredSplendidHot_TryNow_Check_WinAmount_AddedToBalance {
	AppiumDriver<MobileElement> driver;
	private MobileElement prq;
	
	@Given("^Chrome browser, valid URL, valid login details, (\\d+) Splendid Hot slot game, try now button, balance to play, spin button and win amount$")
	public void chrome_browser_valid_URL_valid_login_details_Splendid_Hot_slot_game_try_now_button_balance_to_play_spin_button_and_win_amount(int arg1) throws Throwable {
	...
	@When("^Open the (\\d+) Splendid Hot slot game by entering the valid URL in browser, click on try now button, click on spin button till player win and check the balance after win$")
	public void open_the_Splendid_Hot_slot_game_by_entering_the_valid_URL_in_browser_click_on_try_now_button_click_on_spin_button_till_player_win_and_check_the_balance_after_win(int arg1) throws Throwable {
		List<MobileElement> balance = driver.findElementsByClassName("android.view.View");
		
		
		////Selecting bet type as 0.4 and default bet value will be 40
	...
	@Then("^Win amount should get added to the balance when the player wins in try now page of (\\d+) Splendid Hot slot game$")
	public void win_amount_should_get_added_to_the_balance_when_the_player_wins_in_try_now_page_of_Splendid_Hot_slot_game(int arg1) throws Throwable {
		Thread.sleep(2000);
	    driver.quit();
	}
}
