...
public class TwentyFrizzlingHot_AutoSpin_Validations_ButtonsBehavior {
	AppiumDriver<MobileElement> driver;

	public TwentyFrizzlingHot_AutoSpin_Validations_ButtonsBehavior() throws InterruptedException {
		this.driver = TwentyFrizzlingHot_URL_Login.getDriver();
		//this.driver = TwentyFrizzlingHot_URL_TryNow.getDriver();
	}
	
	@Given("^Chrome browser, valid URL, valid login details, (\\d+) Frizzling Hot game, balance, spin button, auto spin button, auto spins option and check for buttons behavior & functionality$")
	public void chrome_browser_valid_URL_valid_login_details_Frizzling_Hot_game_balance_spin_button_auto_spin_button_auto_spins_option_and_check_for_buttons_behavior_functionality(int arg1) throws Throwable {
		WebDriverWait wait = new WebDriverWait(driver, 80);
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("hud_Hud_txtBalance1"))); 
	}

	@When("^Open the (\\d+) Frizzling Hot game by entering the valid URL in browser, enter the valid login details, transfer the balance, click on auto spin option under auto spin drop down and check behavior of different buttons in different scenarios$")
	public void open_the_Frizzling_Hot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_transfer_the_balance_click_on_auto_spin_option_under_auto_spin_drop_down_and_check_behavior_of_different_buttons_in_different_scenarios(int arg1) throws Throwable {
		WebDriverWait wait = new WebDriverWait(driver, 80);
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("hud_Hud_txtBalance1"))); 
	...
	@Then("^Bet value & Credit value buttons should not be clickable in (\\d+) Frizzling hot slot game$")
	public void bet_value_Credit_value_buttons_should_not_be_clickable_in_Frizzling_hot_slot_game(int arg1) throws Throwable {
		Thread.sleep(1000);
	}

	@Then("^Home and Info buttons should not be clickable in (\\d+) Frizzling hot slot game$")
	public void home_and_Info_buttons_should_not_be_clickable_in_Frizzling_hot_slot_game(int arg1) throws Throwable {
		Thread.sleep(1000);
	}

	@Then("^Spin button should not visible & Stop button should be visible and clickable in (\\d+) Frizzling hot slot game$")
	public void spin_button_should_not_visible_Stop_button_should_be_visible_and_clickable_in_Frizzling_hot_slot_game(int arg1) throws Throwable {
		Thread.sleep(1000);
	}

	@Then("^Spin button should be visble after clicking on stop button in (\\d+) Frizzling hot slot game$")
	public void spin_button_should_be_visble_after_clicking_on_stop_button_in_Frizzling_hot_slot_game(int arg1) throws Throwable {
		driver.quit();
	}
}