...
public class TwentyFrizzlingHot_Balance_CreditBalance_For_IncrementalAndDecremental_CreditValue {
	AppiumDriver<MobileElement> driver;

	public TwentyFrizzlingHot_Balance_CreditBalance_For_IncrementalAndDecremental_CreditValue() throws InterruptedException {
		this.driver = TwentyFrizzlingHot_URL_Login.getDriver();
		//this.driver = TwentyFrizzlingHot_URL_TryNow.getDriver();
		}
	
	@Given("^Chrome browser, valid URL, valid login details, (\\d+) Frizzling Hot slot game, increment, decrement and balance, balance in credit form, creditvalues$")
	public void chrome_browser_valid_URL_valid_login_details_Frizzling_Hot_slot_game_increment_decrement_and_balance_balance_in_credit_form_creditvalues(int arg1) throws Throwable {
	   
	}

	@When("^Open the (\\d+) Frizzling Hot slot game by entering the valid URL in browser, enter the valid login details, inc and dec credit balance and verify the balance in credit form and verify the balance$")
	public void open_the_Frizzling_Hot_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_inc_and_dec_credit_balance_and_verify_the_balance_in_credit_form_and_verify_the_balance(int arg1) throws Throwable {
	...
	@Then("^Credit balance for all credit values should be same as balance multiplies by credit value are assigned respectively in (\\d+) Frizzling Hot slot game$")
	public void credit_balance_for_all_credit_values_should_be_same_as_balance_multiplies_by_credit_value_are_assigned_respectively_in_Frizzling_Hot_slot_game(int arg1) throws Throwable {
	    Thread.sleep(2000);
	    driver.quit();
	}
}
