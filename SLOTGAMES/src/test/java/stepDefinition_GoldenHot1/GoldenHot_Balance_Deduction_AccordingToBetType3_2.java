...
public class GoldenHot_Balance_Deduction_AccordingToBetType3_2 {
	AppiumDriver<MobileElement> driver;

	public GoldenHot_Balance_Deduction_AccordingToBetType3_2() throws InterruptedException {
	...
	@When("^Open the Golden hot slot game by entering the valid URL in browser, enter the valid login details, select the bet type as (\\d+) from bet type drop down, click on spin button and check the balance$")
	public void open_the_Golden_hot_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_select_the_bet_type_as_from_bet_type_drop_down_click_on_spin_button_and_check_the_balance(int arg1) throws Throwable {
	...
	@Then("^Balance should get deducted by (\\d+) if bet type is selected as (\\d+)\\.(\\d+) & bet value as (\\d+) in Golden Hot game$")
	public void balance_should_get_deducted_by_if_bet_type_is_selected_as_bet_value_as_in_Golden_Hot_game(int arg1, int arg2, int arg3, int arg4) throws Throwable {
		Thread.sleep(2000);
	    driver.quit();
	}
}
