...
public class TwentyBlazingHot_Balance_InsufficiantBalance_Validation_Bet_And_CreditValue_Decrement {
	AppiumDriver<MobileElement> driver;

	
	@Given("^Chrome browser, valid URL, valid login details, (\\d+) Blazing Hot slot game, balance counter, Auto spin button, validation message, credit types and bet types$")
	public void chrome_browser_valid_URL_valid_login_details_Blazing_Hot_slot_game_balance_counter_Auto_spin_button_validation_message_credit_types_and_bet_types(int arg1) throws Throwable {
	...
	@When("^Open the (\\d+) Blazing Hot slot game by entering the valid URL in browser, enter the valid login details, transfer the balance, click on Auto spin button till balance counter turns less than betvalues and check the insufficient validation message and reduce the bet and credit values to resume the play\\.$")
	...
	@Then("^Player should be blocked from playing the game when there is less amount in balance counter and reducing the credit and bet types player is enable to continue slot game in (\\d+) Blazing Hot game$")
	public void player_should_be_blocked_from_playing_the_game_when_there_is_less_amount_in_balance_counter_and_reducing_the_credit_and_bet_types_player_is_enable_to_continue_slot_game_in_Blazing_Hot_game(int arg1) throws Throwable {
		Thread.sleep(2000);
	    driver.quit();
	}
}