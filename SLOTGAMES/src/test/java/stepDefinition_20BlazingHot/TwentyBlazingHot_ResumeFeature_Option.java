...
public class TwentyBlazingHot_ResumeFeature_Option {
	AppiumDriver<MobileElement> driver;

	public TwentyBlazingHot_ResumeFeature_Option() throws InterruptedException {
		this.driver = TwentyBlazingHot_URL_Login.getDriver();
		//this.driver = TwentyBlazingHot_URL_TryNow.getDriver();	
	}
	
	@Given("^Chrome browser, valid URL, valid login details, (\\d+) Blazing Hot slot slot game, bet type, denomination, balance, spin button, Gamble screen, new browser session\\.$")
	public void chrome_browser_valid_URL_valid_login_details_Blazing_Hot_slot_slot_game_bet_type_denomination_balance_spin_button_Gamble_screen_new_browser_session(int arg1) throws Throwable {
	    
	}

	@When("^Open the (\\d+) Blazing Hot slot slot game by entering the valid URL in browser, enter the valid login details, transfer the balance, select bet type & denomination, click on spin button till win appears and gamble screen in switched and click on red button and close the browser and again login to the game with new browser$")
	public void open_the_Blazing_Hot_slot_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_transfer_the_balance_select_bet_type_denomination_click_on_spin_button_till_win_appears_and_gamble_screen_in_switched_and_click_on_red_button_and_close_the_browser_and_again_login_to_the_game_with_new_browser(int arg1) throws Throwable {
	...
	@Then("^Resume feature should enabled and land on the gamble screen of (\\d+) Blazing Hot slot game$")
	public void resume_feature_should_enabled_and_land_on_the_gamble_screen_of_Blazing_Hot_slot_game(int arg1) throws Throwable {
		Thread.sleep(1000);
	}

	@Then("^On resuming to gamble screen, collect the amount and verify with main  balance addition and win meter amount same as gamble amount in gamble page in  (\\d+) Blazing Hot slot game$")
	public void on_resuming_to_gamble_screen_collect_the_amount_and_verify_with_main_balance_addition_and_win_meter_amount_same_as_gamble_amount_in_gamble_page_in_Blazing_Hot_slot_game(int arg1) throws Throwable {
		Thread.sleep(2000);
	    driver.quit();
	}
}
