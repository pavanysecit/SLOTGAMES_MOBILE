...
public class BlazingHot_Check_BetType_BetValue_DropDown1 {
	...
	@Then("^Bet value should get changed to (\\d+)\\.(\\d+),(\\d+)\\.(\\d+),(\\d+),(\\d+),(\\d+) if the bet type selected as (\\d+)\\.(\\d+) from the drop down in Blazing Hot slot game$")
