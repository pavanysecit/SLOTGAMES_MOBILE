...
public class FourtyGlowingHot_Check_BetType_BetValue_DropDown_For_IncrementalAndDecremental_CreditValue {
AppiumDriver<MobileElement> driver;
	
	public FourtyGlowingHot_Check_BetType_BetValue_DropDown_For_IncrementalAndDecremental_CreditValue() throws InterruptedException {
		this.driver = FourtyGlowingHot_URL_Login.getDriver();
		//this.driver = FourtyGlowingHot_URL_TryNow.getDriver();
		}
	
	@Given("^Chrome browser, valid URL, valid login details, (\\d+) Glowing Hot slot game, increment, decrement and betvalue amount$")
	public void chrome_browser_valid_URL_valid_login_details_Glowing_Hot_slot_game_increment_decrement_and_betvalue_amount(int arg1) throws Throwable {
	   
	}

	@When("^Open the (\\d+) Glowing Hot slot game by entering the valid URL in browser, enter the valid login details, inc and dec credit and verify the betvalue assigned respectively$")
	public void open_the_Glowing_Hot_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_inc_and_dec_credit_and_verify_the_betvalue_assigned_respectively(int arg1) throws Throwable {
	...
	@Then("^Bet amount for all credit values should be assigned respectively in (\\d+) Glowing Hot slot game$")
	public void bet_amount_for_all_credit_values_should_be_assigned_respectively_in_Glowing_Hot_slot_game(int arg1) throws Throwable {
		Thread.sleep(2000);
	    driver.quit();
	}
}
