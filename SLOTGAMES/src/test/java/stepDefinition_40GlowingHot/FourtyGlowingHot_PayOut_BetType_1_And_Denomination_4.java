...
public class FourtyGlowingHot_PayOut_BetType_1_And_Denomination_4 {
AppiumDriver<MobileElement> driver;
	
	public FourtyGlowingHot_PayOut_BetType_1_And_Denomination_4() throws InterruptedException {
		this.driver = FourtyGlowingHot_URL_Login.getDriver();
		//this.driver = FourtyGlowingHot_URL_TryNow.getDriver();
		}
	
	@Given("^Chrome browser, valid URL, valid login details, (\\d+) glowing hot slot game, icon to open payout table, all symbols, max amount on each symbol like (\\d+) n etc, back button, bet type (\\d+)\\.(\\d+) and denomination value (\\d+)$")
	public void chrome_browser_valid_URL_valid_login_details_glowing_hot_slot_game_icon_to_open_payout_table_all_symbols_max_amount_on_each_symbol_like_n_etc_back_button_bet_type_and_denomination_value(int arg1, int arg2, int arg3, int arg4, int arg5) throws Throwable {
		WebDriverWait wait = new WebDriverWait(driver, 80);
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("hud_Hud_txtBalance1"))); 
	}

	@When("^Open the (\\d+) Glowing hot slot game by entering the valid URL in browser, enter the valid login details, transfer the balance, click on icon to open payout table, check payout amount on each symbol like (\\d+) n etc, click on back button, select bet type as (\\d+)\\.(\\d+) & denomination  value (\\d+) and check payout amount$")
	public void open_the_Glowing_hot_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_transfer_the_balance_click_on_icon_to_open_payout_table_check_payout_amount_on_each_symbol_like_n_etc_click_on_back_button_select_bet_type_as_denomination_value_and_check_payout_amount(int arg1, int arg2, int arg3, int arg4, int arg5) throws Throwable {
		...
		if(me.getId().equals("16")){
			Fsymbol = me.getText();
			String expected44 = Fsymbol;
			String actual44 = "300 YSI";
		...
		if(me.getId().equals("17")){
			...
			System.out.println("Max Payout Value of 4 7777 symbols for denomination 4 is : " +F1symbol);
		...
		if(me.getId().equals("18")){
			...
			System.out.println("Max Payout Value of 3 777 symbols for denomination 4 is : " +F2symbol);
		...
		if(me.getId().equals("19")){
			...
			System.out.println("Max Payout Value of 2 77 symbols for denomination 4 is : " +F3symbol);
		...
		if(me.getId().equals("38")){
			...
			System.out.println("Payout Value of purple scatter symbol for denomination 4 is: " +pscatter);
	...
	@Then("^System should display the Payout max amount on each symbol like (\\d+) n etc and payout max amount should get changed whenever player changes the bet type to (\\d+)\\.(\\d+) and denomination value (\\d+) in (\\d+) glowing hot slot game$")
	public void system_should_display_the_Payout_max_amount_on_each_symbol_like_n_etc_and_payout_max_amount_should_get_changed_whenever_player_changes_the_bet_type_to_and_denomination_value_in_glowing_hot_slot_game(int arg1, int arg2, int arg3, int arg4, int arg5) throws Throwable {
		Thread.sleep(3000);
		driver.quit();
	}
}