...
public class FourtyGlowingHot_Balance_FullAmount_AddedToGame_And_Reload_Option {
AppiumDriver<MobileElement> driver;
	
	public FourtyGlowingHot_Balance_FullAmount_AddedToGame_And_Reload_Option() throws InterruptedException {
		this.driver = FourtyGlowingHot_URL_Login.getDriver();
		//this.driver = FourtyGlowingHot_URL_TryNow.getDriver();
		}
	
	@Given("^Chrome browser, valid URL, valid login details, (\\d+) Glowing Hot slot game, Full amount transfer from account to game\\.$")
	public void chrome_browser_valid_URL_valid_login_details_Glowing_Hot_slot_game_Full_amount_transfer_from_account_to_game(int arg1) throws Throwable {
	   
	}

	@When("^Open the (\\d+) Glowing Hot slot game by entering the valid URL in browser, enter the valid login details, select game and transfer full amount same amount has to reflect in game balance, play and reload the game$")
	public void open_the_Glowing_Hot_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_select_game_and_transfer_full_amount_same_amount_has_to_reflect_in_game_balance_play_and_reload_the_game(int arg1) throws Throwable {
	...
	@Then("^Remaining balance should be transfered to main account balance in (\\d+) Glowing Hot slot game$")
	public void remaining_balance_should_be_transfered_to_main_account_balance_in_Glowing_Hot_slot_game(int arg1) throws Throwable {
		Thread.sleep(2000);
	    driver.quit();
	}
}
