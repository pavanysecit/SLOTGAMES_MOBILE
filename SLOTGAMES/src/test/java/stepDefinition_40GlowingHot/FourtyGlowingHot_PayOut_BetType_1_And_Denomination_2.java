...
public class FourtyGlowingHot_PayOut_BetType_1_And_Denomination_2 {
AppiumDriver<MobileElement> driver;
	
	public FourtyGlowingHot_PayOut_BetType_1_And_Denomination_2() throws InterruptedException {
	...
	@Given("^Chrome browser, valid URL, valid login details, (\\d+) glowing hot slot game, icon to open payout table, all symbols, max amount on each symbol like cherry n etc, back button, bet type (\\d+)\\.(\\d+) and denomination value (\\d+)\\.(\\d+)$")
	public void chrome_browser_valid_URL_valid_login_details_glowing_hot_slot_game_icon_to_open_payout_table_all_symbols_max_amount_on_each_symbol_like_cherry_n_etc_back_button_bet_type_and_denomination_value(int arg1, int arg2, int arg3, int arg4, int arg5) throws Throwable {
	    driver.findElement(By.xpath("/html/body/div[3]/div[1]/ui-view/section/section[1]/div/div/div[2]/div[3]/ul/li[1]/div[1]/div/span")).click();
		Thread.sleep(5000);
	...
	public void open_the_Glowing_hot_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_transfer_the_balance_click_on_golden_hot_slot_game_click_on_icon_to_open_payout_table_check_payout_amount_on_each_symbol_like_cherry_n_etc_click_on_back_button_select_bet_type_as_denomination_value_and_check_payout_amount(int arg1, int arg2, int arg3, int arg4, int arg5) throws Throwable {
		...
		// Checking payout max amount for each symbol when bet type is 0.01 and denomination is 0.8
		...
		if(me.getId().equals("23")){
			...
			System.out.println("Payout Value of 5 grapes symbols for denomination 0.8 is: " +grape);
		...
		if(me.getId().equals("24")){
			...
			System.out.println("Payout Value of 4 grapes symbols for denomination 0.8 is: " +grape1);
		...
		if(me.getId().equals("25")){
			...
			System.out.println("Payout Value of 3 grapes symbols for denomination 0.8 is: " +grape2);
		...
		if(me.getId().equals("26")){
			...
			System.out.println("Payout Value of 5 Bell symbols for denomination 0.8 is: " +bell);
		...
		if(me.getId().equals("27")){
			...
			System.out.println("Payout Value of 4 Bell symbols for denomination 0.8 is: " +bell1);
		...
		if(me.getId().equals("28")){
			...
			System.out.println("Payout Value of 3 Bell symbols for denomination 0.8 is: " +bell2);
		...
		if(me.getId().equals("29")){
			...
			System.out.println("Payout Value of 5 cherry symbols for denomination 0.8 is: " +cherry);
		...
		if(me.getId().equals("30")){
			...
			System.out.println("Payout Value of 4 cherry symbols for denomination 0.8 is: " +cherry1);
		...
		if(me.getId().equals("31")){
			...
			System.out.println("Payout Value of 3 cherry symbols for denomination 0.8 is: " +cherry2);
		...
		if(me.getId().equals("38")){
			...
			System.out.println("Payout Value of purple scatter symbol for denomination 0.8 is: " +pscatter);
	...
	@Then("^System should display the Payout max amount on each symbol like cherry n etc and payout max amount should get changed whenever player changes the bet type to (\\d+)\\.(\\d+) and denomination value (\\d+)\\.(\\d+) in (\\d+) glowing hot slot game$")
	public void system_should_display_the_Payout_max_amount_on_each_symbol_like_cherry_n_etc_and_payout_max_amount_should_get_changed_whenever_player_changes_the_bet_type_to_and_denomination_value_in_glowing_hot_slot_game(int arg1, int arg2, int arg3, int arg4, int arg5) throws Throwable {
		Thread.sleep(3000);
		driver.quit();
	}
}