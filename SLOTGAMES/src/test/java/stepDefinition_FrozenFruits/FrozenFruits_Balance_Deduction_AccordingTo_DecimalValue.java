...
public class FrozenFruits_Balance_Deduction_AccordingTo_DecimalValue {
	AppiumDriver<MobileElement> driver;

	
	@Given("^Chrome browser, valid URL, valid login details, Frozen Fruits slot game, bet type as (\\d+)\\.(\\d+), mimimum bet value, balance in decimal value and spin button$")
	public void chrome_browser_valid_URL_valid_login_details_Frozen_Fruits_slot_game_bet_type_as_mimimum_bet_value_balance_in_decimal_value_and_spin_button(int arg1, int arg2) throws Throwable {
	...
	@When("^Open the Frozen Fruits slot game by entering the valid URL in browser, enter the valid login details, select the bet type as (\\d+)\\.(\\d+), select the bet value as (\\d+)\\.(\\d+), click on spin button and check the balance in decimal$")
	public void open_the_Frozen_Fruits_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_select_the_bet_type_as_select_the_bet_value_as_click_on_spin_button_and_check_the_balance_in_decimal(int arg1, int arg2, int arg3, int arg4) throws Throwable {
	...
	@Then("^Balance should get deducted by decimal value, as bet type is selected as (\\d+)\\.(\\d+) and bet value as (\\d+)\\.(\\d+) in Frozen Fruits game$")
	public void balance_should_get_deducted_by_decimal_value_as_bet_type_is_selected_as_and_bet_value_as_in_Frozen_Fruits_game(int arg1, int arg2, int arg3, int arg4) throws Throwable {
		 Thread.sleep(1000);
	}

	@Then("^After deducting balance, switch to credit mode and spin and check balance is deducting according to credit value in Frozen Fruits game$")
	public void after_deducting_balance_switch_to_credit_mode_and_spin_and_check_balance_is_deducting_according_to_credit_value_in_Frozen_Fruits_game() throws Throwable {
	   Thread.sleep(1000);
	   driver.quit();
	}
}