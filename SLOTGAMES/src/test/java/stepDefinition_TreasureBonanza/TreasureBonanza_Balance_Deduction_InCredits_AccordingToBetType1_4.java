...
public class TreasureBonanza_Balance_Deduction_InCredits_AccordingToBetType1_4 {
	AppiumDriver<MobileElement> driver;

	public TreasureBonanza_Balance_Deduction_InCredits_AccordingToBetType1_4() throws InterruptedException {
	...
	@When("^Open the Treasure Bonanza game by entering the valid URL in browser, enter the valid login details, select the bet type as (\\d+)\\.(\\d+), select the bet value as TWO HUNDRED, click on spin button and check the balance in credits$")
	public void open_the_Treasure_Bonanza_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_select_the_bet_type_as_select_the_bet_value_as_TWO_HUNDRED_click_on_spin_button_and_check_the_balance_in_credits(int arg1, int arg2) throws Throwable {
	...
	@Then("^Balance should get deducted by TWO HUNDRED, as bet type is selected as (\\d+)\\.(\\d+) and bet value as (\\d+) in Treasure Bonanza game$")
	public void balance_should_get_deducted_by_TWO_HUNDRED_as_bet_type_is_selected_as_and_bet_value_as_in_Treasure_Bonanza_game(int arg1, int arg2, int arg3) throws Throwable {
		Thread.sleep(2000);
	    driver.quit();
	}
}
