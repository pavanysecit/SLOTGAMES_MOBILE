...
public class Hot777_Check_SessionExpired_Messages {
	AppiumDriver<MobileElement> driver;

	public Hot777_Check_SessionExpired_Messages() throws InterruptedException {
		this.driver = Hot777_URL_Login.getDriver();
		//this.driver = Hot777_URL_TryNow.getDriver();
	}
	
	@Given("^Chrome browser, valid URL, valid login details, Hot (\\d+) slot game, balance, spin button, full screen, landscape mode ,session terminated messages$")
	public void chrome_browser_valid_URL_valid_login_details_Hot_slot_game_balance_spin_button_full_screen_landscape_mode_session_terminated_messages(int arg1) throws Throwable {
	   
	}

	@When("^Open the Hot (\\d+) slot game by entering the valid URL in browser, enter the valid login details, and full screen and change orientation of the screen, idle for more (\\d+)mins$")
	public void open_the_Hot_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_and_full_screen_and_change_orientation_of_the_screen_idle_for_more_mins(int arg1, int arg2) throws Throwable {
	...
	@Then("^User is addressed with valid error message on the screen 'Session Expired' in Hot (\\d+) slot game$")
	public void user_is_addressed_with_valid_error_message_on_the_screen_Session_Expired_in_Hot_slot_game(int arg1) throws Throwable {
		Thread.sleep(1000);
	}
