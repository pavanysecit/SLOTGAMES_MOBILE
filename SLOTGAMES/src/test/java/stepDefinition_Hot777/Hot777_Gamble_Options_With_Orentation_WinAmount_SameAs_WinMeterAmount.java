...
public class Hot777_Gamble_Options_With_Orentation_WinAmount_SameAs_WinMeterAmount {
	AppiumDriver<MobileElement> driver;

	public Hot777_Gamble_Options_With_Orentation_WinAmount_SameAs_WinMeterAmount() throws InterruptedException {
		this.driver = Hot777_URL_Login.getDriver();
		//this.driver = Hot777_URL_TryNow.getDriver();
	}
	
	@Given("^Chrome browser, valid URL, valid login details, Hot (\\d+) slot game, balance, spin button, win amount, gamble button, color win option, game play in landscape mode in gamble page, gamble status$")
	public void chrome_browser_valid_URL_valid_login_details_Hot_slot_game_balance_spin_button_win_amount_gamble_button_color_win_option_game_play_in_landscape_mode_in_gamble_page_gamble_status(int arg1) throws Throwable {
		WebDriverWait wait = new WebDriverWait(driver, 80);
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("hud_Hud_txtBalance1"))); 
	}

	@When("^Open the Hot (\\d+) slot game by entering the valid URL in browser, enter the valid login details, click on spin button till player wins, click on gamble button, check the play options and play in landscape mode in gamble page and when user wins check the win meter at main page is same as gamble win amount$")
	public void open_the_Hot_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_click_on_spin_button_till_player_wins_click_on_gamble_button_check_the_play_options_and_play_in_landscape_mode_in_gamble_page_and_when_user_wins_check_the_win_meter_at_main_page_is_same_as_gamble_win_amount(int arg1) throws Throwable {
	...
	@Then("^Gamble page should provide multiple options for the user to play and enabled to play in landscape mode and check the win meter in main page in Hot (\\d+) slot game$")
	public void gamble_page_should_provide_multiple_options_for_the_user_to_play_and_enabled_to_play_in_landscape_mode_and_check_the_win_meter_in_main_page_in_Hot_slot_game(int arg1) throws Throwable {
		Thread.sleep(1000);
	    driver.quit();
	}
}