...
public class Hot777_AutoSpin_Validations_NetworkInteruptions {
	AppiumDriver<MobileElement> driver;

	public Hot777_AutoSpin_Validations_NetworkInteruptions() throws InterruptedException {
		this.driver = Hot777_URL_Login.getDriver();
		//this.driver = Hot777_URL_TryNow.getDriver();
	}
	
	@Given("^Chrome browser, valid URL, valid login details, Hot (\\d+) slot game, balance, spin button, wifi turnOFF and ON, network messages$")
	public void chrome_browser_valid_URL_valid_login_details_Hot_slot_game_balance_spin_button_wifi_turnOFF_and_ON_network_messages(int arg1) throws Throwable {
		WebDriverWait wait = new WebDriverWait(driver, 60);
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("hud_Hud_txtBalance1"))); 
	}

	@When("^Open the Hot (\\d+) slot game by entering the valid URL in browser, enter the valid login details, and launch the game, spin autospin and turn OFF wifi and check for valid message\\. and turn on wifi for autospin to continued$")
	public void open_the_Hot_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_and_launch_the_game_spin_autospin_and_turn_OFF_wifi_and_check_for_valid_message_and_turn_on_wifi_for_autospin_to_continued(int arg1) throws Throwable {
	...
	@Then("^User is addressed with valid error message 'No Internet' for network interuptions in Hot (\\d+) slot game$")
	public void user_is_addressed_with_valid_error_message_No_Internet_for_network_interuptions_in_Hot_slot_game(int arg1) throws Throwable {
		Thread.sleep(1000);
	}

	@Then("^User is addressed with valid error message 'Disconnected from Server' for longer duration network interuptions in Hot (\\d+) slot game$")
	public void user_is_addressed_with_valid_error_message_Disconnected_from_Server_for_longer_duration_network_interuptions_in_Hot_slot_game(int arg1) throws Throwable {
		Thread.sleep(1000);
		driver.quit();
	}
}