...
public class GreatEgypt_Balance_InsufficiantBalance_Validation {
	AppiumDriver<MobileElement> driver;

	
	@Given("^Chrome browser, valid URL, valid login details, Great Egypt slot game, balance, spin button and validation message, button behavior,$")
	...
	@Then("^Player should be blocked from playing the game when there is no balance in the account and buttons are inactive state in Great Egypt game$")
	public void player_should_be_blocked_from_playing_the_game_when_there_is_no_balance_in_the_account_and_buttons_are_inactive_state_in_Great_Egypt_game() throws Throwable {
	   Thread.sleep(1000);
	   driver.quit();
	}
}
