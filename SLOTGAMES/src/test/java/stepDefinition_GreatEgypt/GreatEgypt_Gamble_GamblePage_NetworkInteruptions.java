...
public class GreatEgypt_Gamble_GamblePage_NetworkInteruptions {
	AppiumDriver<MobileElement> driver;

	public GreatEgypt_Gamble_GamblePage_NetworkInteruptions() throws InterruptedException {
		this.driver = GreatEgypt_URL_Login.getDriver();
		//this.driver = GreatEgypt_URL_TryNow.getDriver();
	}
	
	@Given("^Chrome browser, valid URL, valid login details, Great Egypt slot game, balance, spin button, gamble screen, wifi turnOFF and ON, network messages$")
	public void chrome_browser_valid_URL_valid_login_details_Great_Egypt_slot_game_balance_spin_button_gamble_screen_wifi_turnOFF_and_ON_network_messages() throws Throwable {
	    
	}

	@When("^Open the Great Egypt slot game by entering the valid URL in browser, enter the valid login details, and launch the game, spin till player wins, selext any gamble option and turn OFF wifi and check for valid message and turn on wifi for gamble to resume if won or go back to game screen$")
	public void open_the_Great_Egypt_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_and_launch_the_game_spin_till_player_wins_selext_any_gamble_option_and_turn_OFF_wifi_and_check_for_valid_message_and_turn_on_wifi_for_gamble_to_resume_if_won_or_go_back_to_game_screen() throws Throwable {
	...
	@Then("^User is addressed with valid error message 'No Internet' for network interuptions in gamble page of Great Egypt slot game$")
	public void user_is_addressed_with_valid_error_message_No_Internet_for_network_interuptions_in_gamble_page_of_Great_Egypt_slot_game() throws Throwable {
	   Thread.sleep(2000);
	   driver.quit();
	}
}