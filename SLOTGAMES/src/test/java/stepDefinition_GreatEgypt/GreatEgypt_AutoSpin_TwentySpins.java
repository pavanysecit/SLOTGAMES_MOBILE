...
public class GreatEgypt_AutoSpin_TwentySpins {
	AppiumDriver<MobileElement> driver;

	public GreatEgypt_AutoSpin_TwentySpins() throws InterruptedException {
		this.driver = GreatEgypt_URL_Login.getDriver();
		//this.driver = GreatEgypt_URL_TryNow.getDriver();
	}
	
	@Given("^Chrome browser, valid URL, valid login details, Great Egypt slot game, balance, spin button, auto spin button, twenty spins option and number of spins left message$")
	public void chrome_browser_valid_URL_valid_login_details_Great_Egypt_slot_game_balance_spin_button_auto_spin_button_twenty_spins_option_and_number_of_spins_left_message() throws Throwable {
	...
	@Then("^Number of spin left should start from (\\d+) to (\\d+) after clicking twenty spins option and should perform (\\d+) spins from (\\d+) to (\\d+) in Great Egypt slot game$")
	public void number_of_spin_left_should_start_from_to_after_clicking_twenty_spins_option_and_should_perform_spins_from_to_in_Great_Egypt_slot_game(int arg1, int arg2, int arg3, int arg4, int arg5) throws Throwable {
	   Thread.sleep(2000);
	   driver.quit();
	}
}