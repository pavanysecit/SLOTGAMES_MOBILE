...
public class HundredGlowingHot_AutoSpin_Validations_ButtonsBehavior {
	AppiumDriver<MobileElement> driver;

	public HundredGlowingHot_AutoSpin_Validations_ButtonsBehavior() throws InterruptedException {
	...
	public void open_the_Glowing_Hot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_transfer_the_balance_click_on_auto_spin_option_under_auto_spin_drop_down_and_check_behavior_of_different_buttons_in_different_scenarios(int arg1) throws Throwable {
		...
		Thread.sleep(2000);
		if(driver.findElements(By.id("hud_CreditPopup20.04")).size() != 0){
			System.out.println("Credit click is performed but value is changed to 0.04 during autoplay. Hence test case failed");
		}else{
		...
		Thread.sleep(2000);
		if(driver.findElements(By.id("hud_BetPopup312")).size() != 0){
