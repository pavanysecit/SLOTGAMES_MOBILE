...
public class GummyLand_Balance_Deduction_AccordingToBetType2_3 {
	AppiumDriver<MobileElement> driver;

	public GummyLand_Balance_Deduction_AccordingToBetType2_3() throws InterruptedException {
	...
	@When("^Open the Gummy Landslot slot game by entering the valid URL in browser, enter the valid login details, select the bet type as (\\d+)\\.(\\d+), select the bet value as (\\d+)\\.(\\d+), click on spin button and check the balance$")
	public void open_the_Gummy_Landslot_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_select_the_bet_type_as_select_the_bet_value_as_click_on_spin_button_and_check_the_balance(int arg1, int arg2, int arg3, int arg4) throws Throwable {
	...
	@Then("^Current Balance should get deducted by (\\d+)\\.(\\d+) as bet type is selected as (\\d+)\\.(\\d+) & bet value as (\\d+)\\.(\\d+) in Gummy Land slot game$")
	public void current_Balance_should_get_deducted_by_as_bet_type_is_selected_as_bet_value_as_in_Gummy_Land_slot_game(int arg1, int arg2, int arg3, int arg4, int arg5, int arg6) throws Throwable {
	    Thread.sleep(1000);
	    driver.quit();
	}
}
