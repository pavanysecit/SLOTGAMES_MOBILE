...
public class BrownieHot_Balance_Deduction_InCredits_AccordingToBetType1_3 {
	AppiumDriver<MobileElement> driver;

	public BrownieHot_Balance_Deduction_InCredits_AccordingToBetType1_3() throws InterruptedException {
		this.driver = BrownieHot_URL_Login.getDriver();
		//this.driver = BrownieHot_URL_TryNow.getDriver();
	}
	
	@Given("^Chrome browser, valid URL, valid login details, Brownie Hot slot game, bet type as (\\d+)\\.(\\d+), bet value as ONE FIFTY, balance in credits and spin button$")
	public void chrome_browser_valid_URL_valid_login_details_Brownie_Hot_slot_game_bet_type_as_bet_value_as_ONE_FIFTY_balance_in_credits_and_spin_button(int arg1, int arg2) throws Throwable {
	...
	@Then("^Balance should get deducted by ONE FIFTY, as bet type is selected as (\\d+)\\.(\\d+) and bet value as (\\d+) in Brownie Hot game$")
	public void balance_should_get_deducted_by_ONE_FIFTY_as_bet_type_is_selected_as_and_bet_value_as_in_Brownie_Hot_game(int arg1, int arg2, int arg3) throws Throwable {
	    Thread.sleep(2000);
	    driver.quit();
	}
}
