...
public class HappyFruits_Balance_Credits_Calculations_Value2 {
	AppiumDriver<MobileElement> driver;

	public HappyFruits_Balance_Credits_Calculations_Value2() throws InterruptedException {
		this.driver = HappyFruits_URL_Login.getDriver();
		//this.driver = HappyFruits_URL_TryNow.getDriver();
	}
	
	@Given("^Chrome browser, valid URL, valid login details, Happy Fruits slot game, balance, credits, denomination drop down and denomination as (\\d+)\\.(\\d+)$")
	public void chrome_browser_valid_URL_valid_login_details_Happy_Fruits_slot_game_balance_credits_denomination_drop_down_and_denomination_as(int arg1, int arg2) throws Throwable {
		WebDriverWait wait1 = new WebDriverWait(driver, 80);
		wait1.until(ExpectedConditions.visibilityOfElementLocated(By.id("hud_Hud_txtBalance1"))); 
	}

	@When("^Open the Happy Fruits slot game by entering the valid URL in browser, enter the valid login details, click on balance, select the denomination from drop down, multiply credit by (\\d+)\\.(\\d+) and check the balance$")
	public void open_the_Happy_Fruits_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_click_on_balance_select_the_denomination_from_drop_down_multiply_credit_by_and_check_the_balance(int arg1, int arg2) throws Throwable {
	...
	@Then("^Balance amound should be same as denomination (\\d+)\\.(\\d+) multiplies by credit in Happy Fruits game$")
	public void balance_amound_should_be_same_as_denomination_multiplies_by_credit_in_Happy_Fruits_game(int arg1, int arg2) throws Throwable {
	    Thread.sleep(2000);
	    driver.quit();
	}
}
