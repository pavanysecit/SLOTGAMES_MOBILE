...
public class FrizzlingHot_PayOut_BetType_1_And_Denomination_2 {
	AppiumDriver<MobileElement> driver;

	public FrizzlingHot_PayOut_BetType_1_And_Denomination_2() throws InterruptedException {
		this.driver = FrizzlingHot_URL_Login.getDriver();
		}
	
	@Given("^Chrome browser, valid URL, valid login details, Frizzling Hot slot game, icon to open payout table, all symbols, max amount on each symbol like mango & etc, back button, bet type (\\d+)\\.(\\d+) and deominations (\\d+)\\.(\\d+)$")
	public void chrome_browser_valid_URL_valid_login_details_Frizzling_Hot_slot_game_icon_to_open_payout_table_all_symbols_max_amount_on_each_symbol_like_mango_etc_back_button_bet_type_and_deominations(int arg1, int arg2, int arg3, int arg4) throws Throwable {
	...
	@When("^Open the Frizzling Hot slot game by entering the valid URL in browser, enter the valid login details, transfer the balance, click on icon to open payout table, check payout amount on each symbol like mango & etc, click on back button, select bet type as (\\d+)\\.(\\d+) & deominations as (\\d+)\\.(\\d+) and check payout amount$")
	public void open_the_Frizzling_Hot_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_transfer_the_balance_click_on_icon_to_open_payout_table_check_payout_amount_on_each_symbol_like_mango_etc_click_on_back_button_select_bet_type_as_deominations_as_and_check_payout_amount(int arg1, int arg2, int arg3, int arg4) throws Throwable {
		...
		if(me.getId().equals("33")){
			...
			String actual8 = "24 YSI";
			System.out.println("Payout Value of 5 Watermelon symbol for denomination 0.2: " +Wmelon);
		...
		if(me.getId().equals("34")){
			...
			System.out.println("Payout Value of 4 Watermelon symbol for denomination 0.2: " +Wmelon1);
		...
		if(me.getId().equals("35")){
			...
			System.out.println("Payout Value of 3 Watermelon symbol for denomination 0.2: " +Wmelon2);
		...
		if(me.getId().equals("39")){
			...
			System.out.println("Payout Value of 5 Plum symbol for denomination 0.2 is: " +Plum);
		...
		if(me.getId().equals("40")){
			...
			String actual15 = "2.4 YSI";
			System.out.println("Payout Value of 4 Plum symbol for denomination 0.2 is: " +Plum1);
		...
		if(me.getId().equals("41")){
			...
			String actual16 = "0.8 YSI";
			System.out.println("Payout Value of 3 Plum symbol for denomination 0.2 is: " +Plum2);
		...
		if(me.getId().equals("45")){
			...
			System.out.println("Payout Value of 5 Mango symbol for denomination 0.2 is: " +Mango);
		...
		if(me.getId().equals("46")){
			...
			String actual21 = "2.4 YSI";
			System.out.println("Payout Value of 4 Mango symbol for denomination 0.2 is: " +Mango1);
		...
		if(me.getId().equals("47")){
			...
			System.out.println("Payout Value of 3 Mango symbol for denomination 0.2 is: " +Mango2);
		...
		if(me.getId().equals("51")){
			...
			String actual26 = "0.2 YSI";
			System.out.println("Payout Value of 2 Cherry symbol for denomination 0.2 is: " +Cherry3);
	...
	@Then("^System should display the Payout max amount on each symbol like mango & etc and payout max amount should get changed whenever player changes the bet type to (\\d+)\\.(\\d+) and deomination to (\\d+)\\.(\\d+) in Frizzling Hot slot game on mobile$")
	public void system_should_display_the_Payout_max_amount_on_each_symbol_like_mango_etc_and_payout_max_amount_should_get_changed_whenever_player_changes_the_bet_type_to_and_deomination_to_in_Frizzling_Hot_slot_game_on_mobile(int arg1, int arg2, int arg3, int arg4) throws Throwable {
		Thread.sleep(2000);
	    driver.quit();
	}
}
