...
public class GoldenCrown_Balance_Deduction_AccordingToBetType3_1 {
AppiumDriver<MobileElement> driver;

	public GoldenCrown_Balance_Deduction_AccordingToBetType3_1() throws InterruptedException {
		this.driver = GoldenCrown_URL_Login.getDriver();
		//this.driver = GoldenCrown_URL_TryNow.getDriver();
	}
	
	@Given("^Chrome browser, valid URL, valid login details, Golden Crown slot game, bet type as (\\d+)\\.(\\d+) from drop down, bet value as ONE, balance and spin button$")
	public void chrome_browser_valid_URL_valid_login_details_Golden_Crown_slot_game_bet_type_as_from_drop_down_bet_value_as_ONE_balance_and_spin_button(int arg1, int arg2) throws Throwable {
	...
	public void open_the_Golden_Crown_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_select_the_bet_type_as_ONE_from_bet_type_drop_down_click_on_spin_button_and_check_the_balance() throws Throwable {
	...
	@Then("^Balance should get deducted by ONE if bet type is selected as (\\d+)\\.(\\d+) in Golden Crown game$")
	public void balance_should_get_deducted_by_ONE_if_bet_type_is_selected_as_in_Golden_Crown_game(int arg1, int arg2) throws Throwable {
	    Thread.sleep(2000);
	    driver.quit();
	}
}
