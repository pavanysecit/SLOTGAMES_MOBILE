...
public class GoldenCrown_PayOut_BetType_3_And_Denomination_2 {
AppiumDriver<MobileElement> driver;

	public GoldenCrown_PayOut_BetType_3_And_Denomination_2() throws InterruptedException {
	...
	@Given("^Chrome browser, valid URL, valid login details, Golden Crown slot game, icon to open payout table, all symbols, max amount on each symbol like bar n etc, back button, bet type (\\d+)\\.(\\d+) and denomination value TWO$")
	public void chrome_browser_valid_URL_valid_login_details_Golden_Crown_slot_game_icon_to_open_payout_table_all_symbols_max_amount_on_each_symbol_like_bar_n_etc_back_button_bet_type_and_denomination_value_TWO(int arg1, int arg2) throws Throwable {
		driver.findElement(By.xpath("/html/body/div[3]/div[1]/ui-view/section/section[1]/div/div/div[2]/div[3]/ul/li[18]/div[1]")).click();
		Thread.sleep(2000);
	...
	@When("^Open the Golden Crown slot game by entering the valid URL in browser, enter the valid login details, transfer the balance, click on icon to open payout table, check payout amount on each symbol like bar n etc, click on back button, select bet type as (\\d+)\\.(\\d+) & denomination  value TWO and check payout amount$")
	public void open_the_Golden_Crown_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_transfer_the_balance_click_on_icon_to_open_payout_table_check_payout_amount_on_each_symbol_like_bar_n_etc_click_on_back_button_select_bet_type_as_denomination_value_TWO_and_check_payout_amount(int arg1, int arg2) throws Throwable {
		...
		// Checking payout max amount for each symbol when bet type is 0.05 and denomination is 2
	...
	@Then("^System should display the Payout max amount on each symbol like bar n etc and payout max amount should get changed whenever player changes the bet type to (\\d+)\\.(\\d+) and denomination value TWO in Golden Crown slot game$")
	public void system_should_display_the_Payout_max_amount_on_each_symbol_like_bar_n_etc_and_payout_max_amount_should_get_changed_whenever_player_changes_the_bet_type_to_and_denomination_value_TWO_in_Golden_Crown_slot_game(int arg1, int arg2) throws Throwable {
		driver.quit();
	}
}
