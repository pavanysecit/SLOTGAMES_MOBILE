...
public class DeluxeFruits_PayOut_BetType_1_And_Denomination_4 {
	AppiumDriver<MobileElement> driver;

	public DeluxeFruits_PayOut_BetType_1_And_Denomination_4() throws InterruptedException {
		this.driver = DeluxeFruits_URL_Login.getDriver();
		}
	
	@Given("^Chrome browser, valid URL, valid login details, Deluxe Fruits slot game, icon to open payout table, all symbols, max amount on each symbol like lemon & grapes, back button, bet type (\\d+)\\.(\\d+) and denomination as (\\d+) on mobile$")
	public void chrome_browser_valid_URL_valid_login_details_Deluxe_Fruits_slot_game_icon_to_open_payout_table_all_symbols_max_amount_on_each_symbol_like_lemon_grapes_back_button_bet_type_and_denomination_as_on_mobile(int arg1, int arg2, int arg3) throws Throwable {
	...
	@When("^Open the Deluxe Fruits slot game by entering the valid URL in browser, enter the valid login details, transfer the balance, click on icon to open payout table, check payout amount on each symbol like lemon & grapes, click on back button, select bet type as (\\d+)\\.(\\d+) & denomination as (\\d+) and check payout amount on mobile$")
	public void open_the_Deluxe_Fruits_slot_game_by_entering_the_valid_URL_in_browser_enter_the_valid_login_details_transfer_the_balance_click_on_icon_to_open_payout_table_check_payout_amount_on_each_symbol_like_lemon_grapes_click_on_back_button_select_bet_type_as_denomination_as_and_check_payout_amount_on_mobile(int arg1, int arg2, int arg3) throws Throwable {
		...
		if(me.getId().equals("28")){
			...
			System.out.println("Payout Value of 3 Seven symbols for denomination 2 is : " +seven1);
			Assert.assertEquals(actual4, expected4);
			Thread.sleep(1000);
		}
		...
		if(me.getId().equals("30")){
			...
			System.out.println("Payout Value of 5 Diamond symbols for denomination 2 is: " +diamond);
		...
		if(me.getId().equals("31")){
			...
			System.out.println("Payout Value of 4 Diamond symbols for denomination 2 is: " +diamond1);
		...
		if(me.getId().equals("32")){
			...
			System.out.println("Payout Value of 3 Diamond symbols for denomination 2 is: " +diamond2);
		...
		if(me.getId().equals("33")){
			...
			System.out.println("Payout Value of 5 Plum symbol for denomination 2: " +Plum);
		...
		if(me.getId().equals("34")){
			...
			System.out.println("Payout Value of 4 Plum symbol for denomination 2: " +Plum1);
		...
		if(me.getId().equals("35")){
			...
			System.out.println("Payout Value of 3 Plum symbol for denomination 2: " +Plum2);
		...
		if(me.getId().equals("36")){
			...
			System.out.println("Payout Value of 5 Double Bar symbol for denomination 2 is: " +Dbar);
		...
		if(me.getId().equals("37")){
			...
			System.out.println("Payout Value of 4 Double Bar symbol for denomination 2 is: " +Dbar1);
		...
		if(me.getId().equals("38")){
			...
			System.out.println("Payout Value of 3 Double Bar symbol for denomination 2 is: " +Dbar2);
		...
		if(me.getId().equals("48")){
			...
			String actual23 = "10 YSI";
			System.out.println("Payout Value of 5 Bell symbol for denomination 2 is: " +bell);
		...
		if(me.getId().equals("49")){
			...
			String actual24 = "3 YSI";
			System.out.println("Payout Value of 4 Bell symbol for denomination 2 is: " +bell1);
		...
		if(me.getId().equals("50")){
			...
			String actual25 = "1.5 YSI";
			System.out.println("Payout Value of 3 Bell symbol for denomination 2 is: " +bell2);
	...
	@Then("^System should display the Payout max amount on each symbol like lemon & grapes and payout max amount should get changed whenever player changes the bet type to (\\d+)\\.(\\d+) and denomination to (\\d+) in Deluxe Fruits slot game on mobile$")
	public void system_should_display_the_Payout_max_amount_on_each_symbol_like_lemon_grapes_and_payout_max_amount_should_get_changed_whenever_player_changes_the_bet_type_to_and_denomination_to_in_Deluxe_Fruits_slot_game_on_mobile(int arg1, int arg2, int arg3) throws Throwable {
		Thread.sleep(2000);
	    driver.quit();
	}
}
